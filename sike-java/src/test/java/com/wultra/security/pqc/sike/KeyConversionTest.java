...
public class KeyConversionTest {
    ...
    public void testConversionToOctets() throws GeneralSecurityException {
        ...
        KeyPair keyPairA = keyGenerator.generateKeyPair(Party.ALICE);
        KeyPair keyPairB = keyGenerator.generateKeyPair(Party.BOB);
        String privKeyOctetsA = ((SidhPrivateKey) keyPairA.getPrivate()).toOctetString();
        String pubKeyOctetsA = ((SidhPublicKey) keyPairA.getPublic()).toOctetString();
        String privKeyOctetsB = ((SidhPrivateKey) keyPairB.getPrivate()).toOctetString();
        String pubKeyOctetsB = ((SidhPublicKey) keyPairB.getPublic()).toOctetString();
        PrivateKey privKeyA = new SidhPrivateKey(sikeParam, Party.ALICE, privKeyOctetsA);
        PublicKey pubKeyA = new SidhPublicKey(sikeParam, pubKeyOctetsA);
        PrivateKey privKeyB = new SidhPrivateKey(sikeParam, Party.BOB, privKeyOctetsB);
        PublicKey pubKeyB = new SidhPublicKey(sikeParam, pubKeyOctetsB);
        assertEquals(privKeyA, keyPairA.getPrivate());
        assertEquals(pubKeyA, keyPairA.getPublic());
        assertEquals(privKeyB, keyPairB.getPrivate());
        assertEquals(pubKeyB, keyPairB.getPublic());
